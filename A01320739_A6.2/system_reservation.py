"""
Hotel Class
Class to create and manage Hotels
"""
...
class Hotel:
    ...
    def create_hotel(self):
        """The method create_hotel method creates
        a new JSON file for the hotel"""
        if not os.path.exists("hotels"):
            os.makedirs("hotels")
        with open(f"hotels/{self.name}.json", "w", encoding="utf-8") as file:
            json.dump({"name": self.name,
                       "rooms": {}, "reservations": []}, file)

    @staticmethod
    def delete_hotel(name):
        """The method delete_hotel method deletes
        the JSON file of the hotel if it exists"""
    ...
    def display_hotel_info(self):
        """The method display_hotel_info method
        reads and displays hotel information from the JSON file."""
    ...
    def modify_hotel_info(self, new_name):
        """The method modify_hotel_info method
        renames the JSON file of the hotel."""
        file_path = f"hotels/{self.name}.json"
    ...
    def reserve_room(self, guest_name, room_number, start_date, end_date):
        """The method reserve_room method adds
        a reservation to the JSON file of the hotel"""
        file_path = f"hotels/{self.name}.json"
        if os.path.exists(file_path):
            with open(file_path, "r+", encoding="utf-8") as file:
                hotel_data = json.load(file)
                if room_number in hotel_data['rooms']:
                    ...
                    print(f"Room number {room_number} does not exist"
                          f"in {self.name} hotel.")
        else:
            print(f"{self.name} hotel does not exist.")

    def cancel_reservation(self, guest_name,
                           room_number, start_date, end_date):
        """The method cancel_reservation method removes
        a reservation from the JSON file of the hotel."""
        file_path = f"hotels/{self.name}.json"
        if os.path.exists(file_path):
            with open(file_path, "r+", encoding="utf-8") as file:
                hotel_data = json.load(file)
